...
// Este archivo contiene toda la lógica JavaScript de la aplicación

// Global Variables
let map = null;
let currentMapType = 'street';
let activeLayers = new Map();
let plugins = new Map(); // Plugin storage
let pluginAPI = null;
let pluginStorage = new Map(); // Plugin-specific storage
...
async function initializeMap() {
    return new Promise((resolve) => {
        try {
            ...
            resolve(); // Continue even if map fails
        }
    });
}
...
function clearAllLayers() {
    // Clear only overlay layers, keep base layer
        ...
        if (saved) {
            ...
            for (const [id, data] of Object.entries(pluginData)) {
                try {
                    ...
                    const pluginData = localStorage.getItem(pluginDataKey);
                    if (pluginData) {
                        pluginStorage.set(id, JSON.parse(pluginData));
                    }
    ...
    try {
        ...
        plugins.forEach((plugin, id) => {
            ...
            pluginData[id] = {
                ...
                outputs: plugin.outputs.slice(-50), // Keep last 50 outputs
    ...
    if (extension === '.js' || extension === '.json' || extension === '.geojson' || extension === '.csv') {
        ...
        if (extension === '.json') {
            try {
                ...
                // Not a config JSON, treat as data
...
async function loadDefaultPlugins() {
    // Load example plugins if none exist
    if (plugins.size === 0) {
...
async function createExamplePlugins() {
    // Example 1: Weather Overlay Plugin
    const weatherPlugin = {
        id: 'example_weather',
...
window.__pluginResult = weatherPlugin.initialize();`,
    ...
    // Example 2: Measurement Tool Plugin
...
function startSystemServices() {
    ...
    // Auto-save plugins every 30 seconds
...
function showThreatAnalysis() {
    // Create threat analysis overlay
...
function showWeatherLayer() {
    ...
    const weatherLayer = L.tileLayer('https://tile.openweathermap.org/map/{layer}/{z}/{x}/{y}.png?appid={apiKey}', {
        layer: 'clouds_new',
        apiKey: 'demo', // Note: Replace with actual API key for production
        attribution: 'Weather data © OpenWeatherMap',
        opacity: 0.6,
        maxZoom: 19
    }).addTo(map);
...
function handleBeforeUnload(event) {
    ...
    // Optional: Show confirmation for unsaved changes
    // event.preventDefault();
    // event.returnValue = '';
}
